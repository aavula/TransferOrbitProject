...
for k = 1:length(t)
    ...
    a_s = ((2*pi*deltaT./((nuM2(k) - nuE(k)))).^2./lambda).^(1/3);
    e_s = (r_E(k).*cos(nuE(k) - sqrt(r_E(k).^2.*cos(nuE(k)).^2+4.*a_s.*(abs(a_s-r_E(k)))))/(-2.*a_s));
    
    if nuE(k) < nuM2(k)
    nus = nuE(k):.0001:nuM2(k);
    elseif nuE(k) > nuM2(k)
        nus = nuM2(k):.0001:nuE(k);
    else 
        continue 
    end 
